...
export const useImageUpload = (): UseImageUploadReturn => {
   ...
   * Upload single image
   */
  const uploadImage = useCallback(async (file: File, accessToken: string): Promise<ImageMetadata> => {
    try {
      setUploading(true);
      setError(null);
      setProgress(null);
   ...
   */
  const uploadMultiple = useCallback(async (files: File[], accessToken: string): Promise<ImageMetadata[]> => {
