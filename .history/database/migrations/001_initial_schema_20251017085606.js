...
async function up(knex) {
  ...
  try {
    ...
    await knex.schema.createTable('users', (table) => {
      ...
      table.enum('role', ['admin', 'analyst', 'viewer', 'api_user']).notNullable().defaultTo('viewer').index();
    ...
    await knex.schema.createTable('images', (table) => {
      ...
      table.enum('analysis_status', ['pending', 'processing', 'completed', 'failed', 'archived'])
    ...
    await knex.schema.createTable('analysis_results', (table) => {
      table.uuid('id').primary().defaultTo(knex.raw('uuid_generate_v4()'));
      table.uuid('image_id').notNullable().unique().index().references('id').inTable('images').onDelete('CASCADE');
      table.uuid('user_id').notNullable().index().references('id').inTable('users').onDelete('CASCADE');
    ...
    await knex.schema.createTable('processing_queue', (table) => {
      table.uuid('id').primary().defaultTo(knex.raw('uuid_generate_v4()'));
      table.uuid('image_id').notNullable().index().references('id').inTable('images').onDelete('CASCADE');
      table.uuid('user_id').notNullable().index().references('id').inTable('users').onDelete('CASCADE');
      table.enum('status', ['queued', 'processing', 'completed', 'failed', 'retrying', 'cancelled'])
        .notNullable().defaultTo('queued').index();
    ...
    await knex.schema.createTable('security_audit', (table) => {
      ...
      table.enum('status', ['success', 'failure', 'partial']).notNullable().defaultTo('success').index();
