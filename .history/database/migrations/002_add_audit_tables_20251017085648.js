...
async function up(knex) {
  ...
  try {
    ...
    await knex.schema.createTable('audit_log_details', (table) => {
      ...
      table.uuid('audit_id').notNullable().references('id').inTable('security_audit').onDelete('CASCADE').index();
    ...
    await knex.schema.createTable('analysis_cache', (table) => {
      ...
      table.uuid('analysis_id').references('id').inTable('analysis_results').onDelete('CASCADE').index();
    ...
    await knex.schema.table('images', (table) => {
      ...
      table.uuid('duplicate_of').nullable().references('id').inTable('images').onDelete('SET NULL').after('is_duplicate');
    });
    ...
    await knex.raw('CREATE INDEX idx_audit_log_details_event_name ON audit_log_details(event_name)');
    ...
    await knex.raw('CREATE INDEX idx_performance_metrics_metric_name ON performance_metrics(metric_name)');
    await knex.raw('CREATE INDEX idx_performance_metrics_recorded_at ON performance_metrics(recorded_at DESC)');
