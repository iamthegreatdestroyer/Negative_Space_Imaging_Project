...
async function validateEnvironment() {
  ...
  if (process.env.NODE_ENV && !['development', 'testing', 'production'].includes(process.env.NODE_ENV)) {
    errors.push(`Invalid NODE_ENV: ${process.env.NODE_ENV}. Must be: development, testing, or production`);
  }
...
async function checkPostgresHealth() {
  ...
  try {
    ...
    const portAccessible = await checkPort(SERVICE_CONFIG.postgres.host, SERVICE_CONFIG.postgres.port, 5000);
    ...
    if (pg && !STATE.testMode) {
      return await retryWithBackoff(async () => {
        ...
        try {
          ...
          log(LOG_LEVELS.OK, `PostgreSQL query successful - Current time: ${result.rows[0].current_time}`);
...
async function checkRedisHealth() {
  ...
  try {
    // First check if port is accessible
    const portAccessible = await checkPort(SERVICE_CONFIG.redis.host, SERVICE_CONFIG.redis.port, 5000);

    if (!portAccessible) {
      throw new Error(`Redis port ${SERVICE_CONFIG.redis.port} is not accessible on ${SERVICE_CONFIG.redis.host}`);
    }
...
function generateReport() {
  ...
  console.log(`  Overall Status: ${STATE.exitCode === 0 ? `${COLORS.GREEN}HEALTHY${COLORS.RESET}` : `${COLORS.RED}DEGRADED${COLORS.RESET}`}`);
  console.log('');
