...
const ALLOWED_FORMATS = ['image/jpeg', 'image/png', 'image/tiff', 'application/dicom', 'image/fits'];
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response<ImageMetadata>) => {
    if (!req.file) {
      throw new ValidationError([{ field: 'image', message: 'Image file is required' }], 'Missing file');
    }

    // Validate metadata
    const { error, value } = uploadSchema.validate(req.body);
    if (error) {
      throw new ValidationError([{ field: 'metadata', message: error.message }], 'Invalid metadata');
    }
...
router.post(
  ...
  asyncHandler(async (req: AuthenticatedRequest, res: Response<AnalysisJob>) => {
    ...
    if (error) {
      throw new ValidationError([{ field: 'analysis', message: error.message }], 'Invalid analysis parameters');
    }
