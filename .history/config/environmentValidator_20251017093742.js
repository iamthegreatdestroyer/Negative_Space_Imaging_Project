  ...
  async validateConnectivity() {
    ...
    if (this.env.DATABASE_URL) {
      try {
        ...
        if (isReachable) {
          this.result.addSuccess('PostgreSQL', `Reachable at ${dbUrl.hostname}:${dbUrl.port || 5432}`);
        } else {
          this.result.addWarning(
            'PostgreSQL',
    ...
    if (this.env.REDIS_URL) {
      try {
        ...
        const isReachable = await this.checkPortAvailability(redisUrl.hostname, redisUrl.port || 6379);

        if (isReachable) {
          this.result.addSuccess('Redis', `Reachable at ${redisUrl.hostname}:${redisUrl.port || 6379}`);
        } else {
          this.result.addWarning(
            'Redis',
    ...
    if (this.env.API_URL) {
      try {
        ...
        if (isReachable) {
          this.result.addSuccess('API Service', `Reachable at ${apiUrl.hostname}:${apiUrl.port || 80}`);
        } else {
          this.result.addWarning(
            'API Service',
